...
const bitsoAPI = new bitso(BITSO_API_KEY, BITSO_API_SECRET, BITSO_URL, "/api/v3")

    function displayError(Exception) {
        console.log(chalk.red("[FAILED]"))
        if(Exception.response!== undefined) {
            console.log(chalk.red(`${Exception.response.status} : ${Exception.response.statusText} ${JSON.stringify(Exception.response.data)}`));
        } else {
    ...
    async function testBalances() {
        ...
        try {
            ...
            if(checkIfSuccessful("BALANCES", response))
    ...
    async function testWithdrawals(currency, amount, address) {
        ...
        try {
            ...
            if(checkIfSuccessful("WITHDRAWALS", response))
                return response.payload;
        }catch(Exception){
            displayError(Exception)
        }
    ...
    async function testOrders(book, side, amount, type) {
        ...
        try {
            ...
            if(checkIfSuccessful("ORDERS", response))
                return response.payload;
        }catch(Exception){
            displayError(Exception)
        }
